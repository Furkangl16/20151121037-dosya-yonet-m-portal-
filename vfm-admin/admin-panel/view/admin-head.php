    ...
    */
                    ...
                    if (!$updater->findUser($postnewusername)
                        ...
                        if ($postnewusermail) {
                            ...
                            if (isset($_POST['usernotif']) && strlen($mailsystem) > 4) {
                                ...
                                $pulito = dirname($url);
                                $pulito = str_replace('/vfm-admin', '', $pulito);
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        */
        ...
        $_CONFIG['upload_reject_extension'] = array_filter(
            explode(",", $postupload_reject_extension)
        );
        $_CONFIG['upload_allow_type'] = array_filter(
            explode(",", $postupload_allow_type)
        );
        ...
        $_CONFIG['admin_color_scheme'] = (isset(
            $_POST['admin_color_scheme']) ? $_POST['admin_color_scheme'] : 'blue'
        );
        ...
        $_CONFIG['show_langname'] = (isset(
            $_POST['show_langname']) ? true : false
        );
        ...
        $_CONFIG['inline_tw'] = (int) $postinlinetw;

        $_CONFIG['upload_enable'] = (
            isset($_POST['upload_enable']) ? true : false
        );

        $_CONFIG['upload_notification_enable'] = (
            isset($_POST['upload_notification_enable']) ? true : false
        );
        ...
        $_CONFIG['delete_dir_enable'] = (
            isset($_POST['delete_dir_enable']) ? true : false
        );
        $_CONFIG['rename_dir_enable'] = (
            isset($_POST['rename_dir_enable']) ? true : false
        );

        $_CONFIG['move_enable'] = (
            isset($_POST['move_enable']) ? true : false
        );
        $_CONFIG['registration_enable'] = (
            isset($_POST['registration_enable']) ? true : false
        );
        $regrole = filter_input(
            INPUT_POST, "registration_role", FILTER_SANITIZE_STRING
        );
        $_CONFIG['registration_role'] = $regrole;

        $reguserfolders = (
            isset($_POST['reguserfolders']) ? json_encode($_POST['reguserfolders']) : false
        );

        $_CONFIG['registration_user_folders'] = $reguserfolders;

        $regquota = filter_input(
            INPUT_POST, "regquota", FILTER_SANITIZE_STRING
        );
        ...
        $_CONFIG['show_folder_counter'] = (isset(
            $_POST['show_folder_counter']) ? true : false
        );
