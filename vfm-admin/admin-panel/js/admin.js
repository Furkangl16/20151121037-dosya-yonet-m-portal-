 ...
 * @version 2.0
 * @license Standard http://codecanyon.net/licenses/standard
 */
...
// Scrollspy with url
//
var url = location.href.replace(/#.*/,'')+'vfm-admin/';
// Find all anchors
...
function updateSingleBar($newclass) {
    $('.pro input:radio').each(function(){
        var colclass = $(this).val();
        $('.progress-single .progress-bar').removeClass(colclass);
    });
    $('.progress-single .progress-bar').addClass($newclass);
}
