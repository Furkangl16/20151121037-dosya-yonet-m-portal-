    ...
    if (md5($alt.$time) === $hash
        ...
        foreach ($getcloud as $pezzo) {
            if ($downloader->checkFile($pezzo) == true) {
                ...
                if ($actions->renameFile($myfile, ".".$dest."/".$basename, $basename)) {
                    $counter++;
                }
            }
        }
        ...
        if ($counter > 0) {
            $_SESSION['success'] = "<strong>".$counter."</strong> "
            .$encodeExplorer->getString("files_moved_to")
            .": <strong>".$cleandest."</strong>";
        }
        echo "ok";
    } else {
        echo "Action expired";
